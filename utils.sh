...
build_rv() {
	...
	for build_mode in "${build_mode_arr[@]}"; do
		...
		if [ "${args[microg_patch]}" ]; then
			local patched_apk="${TEMP_DIR}/${app_name_l}-revanced-extended-mnml-${version}-${arch}-${build_mode}.apk"
		else
			local patched_apk="${TEMP_DIR}/${app_name_l}-revanced-extended-mnml-${version}-${arch}.apk"
		fi
		if [ ! -f "$stock_apk" ]; then
			if [ "$dl_from" = apkmirror ]; then
